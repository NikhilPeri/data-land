...
from app.datasets.proline.games import ProlineGames
...
def setup_db():
    engine = Database.get_engine('test')
    ProlineGames.metadata.create_all(engine)
    ProlineTickets.metadata.create_all(engine)

    yield

    ProlineGames.metadata.drop_all(engine)
    ProlineTickets.metadata.drop_all(engine)
...
def test_fetches_and_creates_new_tickets():
    mock_reponse
    UpdateTickets.perform('test')

    db = Database.get_session('test')
